...
{-| Implementation of the Alcor Query2 server.
...
import qualified Alcor.ConstantUtils as ConstantUtils (unFrozenSet)
import Alcor.Errors
import qualified Alcor.Path as Path
import Alcor.Daemon
import Alcor.Daemon.Utils (handleMasterVerificationOptions)
import Alcor.Objects
import Alcor.Objects.Lens (configFiltersL)
import qualified Alcor.Config as Config
import Alcor.ConfigReader
import Alcor.BasicTypes
import Alcor.JQueue
import Alcor.JQScheduler
import Alcor.JSON (TimeAsDoubleJSON(..), alterContainerL, lookupContainer)
import Alcor.Locking.Locks (ClientId(..), ClientType(ClientOther))
import Alcor.Logging
import Alcor.Luxi
import qualified Alcor.Query.Language as Qlang
import qualified Alcor.Query.Cluster as QCluster
import Alcor.Path ( queueDir, jobQueueLockFile, jobQueueDrainFile )
import Alcor.Rpc
import qualified Alcor.Query.Exec as Exec
import Alcor.Query.Query
import Alcor.Query.Filter (makeSimpleFilter)
import Alcor.THH.HsRPC (runRpcClient, RpcClientMonad)
import Alcor.Types
import qualified Alcor.UDSServer as U (Handler(..), listener)
import Alcor.Utils ( lockFile, exitIfBad, exitUnless, watchFile
                    , safeRenameFile, newUUID, isUUID )
import Alcor.Utils.Monad (orM)
import Alcor.Utils.MVarLock
import qualified Alcor.Version as Version
import Alcor.WConfd.Client ( getWConfdClient, withLockedConfig, writeConfig
                            , cleanupLocks)
...
withLockedWconfdConfig
  :: (MonadBase IO m, MonadError AlcorException m)
  => ClientId
...
handleQuery :: [Qlang.ItemType -> Qlang.FilterField] -- ^ Fields to put into
            ...
            -> IO (GenericResult AlcorException JSValue)
...
luxiExec
    :: LuxiConfig
    -> LuxiOp
    -> IO (Bool, GenericResult AlcorException JSValue)
