...
import Alcor.ConstantUtils (PythonChar(..), FrozenSet, Protocol(..),
                             buildVersion)
import qualified Alcor.ConstantUtils as ConstantUtils
import qualified Alcor.HTools.Types as Types
import Alcor.HTools.Types (AutoRepairResult(..), AutoRepairType(..))
import Alcor.Logging (SyslogUsage(..))
import qualified Alcor.Logging as Logging (syslogUsageToRaw)
import qualified Alcor.Runtime as Runtime
import Alcor.Runtime (AlcorDaemon(..), MiscGroup(..), AlcorGroup(..),
                       ExtraLogReason(..))
import Alcor.THH (PyValueEx(..))
import Alcor.Types
import qualified Alcor.Types as Types
import Alcor.Confd.Types (ConfdRequestType(..), ConfdReqField(..),
                           ConfdReplyStatus(..), ConfdNodeRole(..),
                           ConfdErrorType(..))
import qualified Alcor.Confd.Types as Types
import qualified Alcor.HTools.Tags.Constants as Tags
...
masterdUser :: String
masterdUser = Runtime.daemonUser AlcorMasterd

masterdGroup :: String
masterdGroup = Runtime.daemonGroup (DaemonGroup AlcorMasterd)

metadUser :: String
metadUser = Runtime.daemonUser AlcorMetad

metadGroup :: String
metadGroup = Runtime.daemonGroup (DaemonGroup AlcorMetad)

rapiUser :: String
rapiUser = Runtime.daemonUser AlcorRapi

rapiGroup :: String
rapiGroup = Runtime.daemonGroup (DaemonGroup AlcorRapi)

confdUser :: String
confdUser = Runtime.daemonUser AlcorConfd

confdGroup :: String
confdGroup = Runtime.daemonGroup (DaemonGroup AlcorConfd)

wconfdUser :: String
wconfdUser = Runtime.daemonUser AlcorWConfd

wconfdGroup :: String
wconfdGroup = Runtime.daemonGroup (DaemonGroup AlcorWConfd)

kvmdUser :: String
kvmdUser = Runtime.daemonUser AlcorKvmd

kvmdGroup :: String
kvmdGroup = Runtime.daemonGroup (DaemonGroup AlcorKvmd)

luxidUser :: String
luxidUser = Runtime.daemonUser AlcorLuxid

luxidGroup :: String
luxidGroup = Runtime.daemonGroup (DaemonGroup AlcorLuxid)

nodedUser :: String
nodedUser = Runtime.daemonUser AlcorNoded

nodedGroup :: String
nodedGroup = Runtime.daemonGroup (DaemonGroup AlcorNoded)

mondUser :: String
mondUser = Runtime.daemonUser AlcorMond

mondGroup :: String
mondGroup = Runtime.daemonGroup (DaemonGroup AlcorMond)
...
confd :: String
confd = Runtime.daemonName AlcorConfd

masterd :: String
masterd = Runtime.daemonName AlcorMasterd

metad :: String
metad = Runtime.daemonName AlcorMetad

mond :: String
mond = Runtime.daemonName AlcorMond

noded :: String
noded = Runtime.daemonName AlcorNoded

wconfd :: String
wconfd = Runtime.daemonName AlcorWConfd

luxid :: String
luxid = Runtime.daemonName AlcorLuxid

rapi :: String
rapi = Runtime.daemonName AlcorRapi

kvmd :: String
kvmd = Runtime.daemonName AlcorKvmd
...
daemonsExtraLogbase =
  ...
  [ (AlcorMond, Map.fromList
                 [ ("access", Runtime.daemonsExtraLogbase AlcorMond AccessLog)
                 , ("error", Runtime.daemonsExtraLogbase AlcorMond ErrorLog)
                 ])
  , (AlcorMetad, Map.fromList
                 [ ("access", Runtime.daemonsExtraLogbase AlcorMetad AccessLog)
                 , ("error", Runtime.daemonsExtraLogbase AlcorMetad ErrorLog)
                 ])
  ]

extraLogreasonAccess :: String
extraLogreasonAccess = Runtime.daemonsExtraLogbase AlcorMond AccessLog

extraLogreasonError :: String
extraLogreasonError = Runtime.daemonsExtraLogbase AlcorMond ErrorLog
...
x509CertSignatureHeader = "X-Alcor-Signature"
...
-- version of Alcor that did not support the 'enabled_disk_templates'
-- so far.
...
-- in the 'Alcor.THH.deCamelCase' function and generate the correct
-- Python name.
rpcTmo_4hrs :: Int
...
-- | Instance specs
--
-- FIXME: these should be associated with 'Alcor.HTools.Types.ISpec'
...
-- * nic* constants are used inside the alcor config
...
cvEnodeversion =
  ("node",
   ...
   "Protocol version mismatch or Alcor version mismatch")
...
-- | List of resources which can be queried using 'Alcor.OpCodes.OpQuery'
...
--
-- FIXME: link with 'Alcor.Query.Language.ResultStatus'
...
hvcDefaults =
  ...
  , (Chroot, Map.fromList [(hvInitScript, PyValueEx "/alcor-chroot")])
  , (Lxc, Map.fromList
          [ (hvCpuMask,             PyValueEx "")
...
instanceCommunicationNicPrefix = "alcor:communication:"
