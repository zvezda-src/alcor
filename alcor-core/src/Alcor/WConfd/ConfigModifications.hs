...
module Alcor.WConfd.ConfigModifications where
...
import Alcor.BasicTypes (GenericResult(..), genericResult, toError)
import Alcor.Constants (lastDrbdPort)
import Alcor.Errors (AlcorException(..))
import Alcor.JSON (Container, GenericContainer(..), alterContainerL
...
import Alcor.Types (AdminState, AdminStateSource)
import Alcor.WConfd.ConfigState (ConfigState, csConfigData, csConfigDataL)
import Alcor.WConfd.Monad (WConfdMonad, modifyConfigWithLock
...
getInstanceByUUID :: ConfigState
                  -> InstanceUUID
                  -> GenericResult AlcorException Instance
...
checkSerial :: SerialNoObject a => a -> a -> GenericResult AlcorException ()
...
replaceIn :: (UuidObject a, TimeStampObjectL a, SerialNoObjectL a)
          ...
          -> a
          -> Container a
          -> GenericResult AlcorException (Container a)
...
updateConfigIfNecessary :: (Monad m, MonadError AlcorException m, Eq a,
...
detachInstanceDisk' :: MonadError AlcorException m
                    => InstanceUUID
                    -> DiskUUID
                    -> ClockTime
                    -> ConfigState
...
detachInstanceDisk' iUuid dUuid ct cs =
  let resetIv :: MonadError AlcorException m
              => Int
              -> [DiskUUID]
              -> ConfigState
              -> m ConfigState
...
removeInstanceDisk' :: MonadError AlcorException m
                    => InstanceUUID
                    -> DiskUUID
                    -> ClockTime
                    -> ConfigState
...
removeInstance iUuid = do
  ...
  let iL = csConfigDataL . configInstancesL . alterContainerL
      ...
      g :: (MonadError AlcorException m, Functor m, MonadFail m) =>
...
setInstanceStatus iUuid m1 m2 m3 = do
  ...
  let modifyInstance = maybe id (instAdminStateL .~) (unMaybeForJSON m1)
      ...
      f :: MonadError AlcorException m => StateT ConfigState m Instance
      f = get >>= (maybe
