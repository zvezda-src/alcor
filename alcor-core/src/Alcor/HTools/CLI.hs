...
binaries, separated into this module since "Alcor.Utils" is
...
module Alcor.HTools.CLI
  ( Options(..)
  , OptType
  , defaultOptions
  , Alcor.HTools.CLI.parseOpts
  , parseOptsInner
  , parseYesNo
  , parseISpecString
  , shTemplate
...
oPrintCommands =
  (Option "C" ["print-commands"]
   (OptArg ((\ f opts -> Ok opts { optShowCmds = Just f }) .
            fromMaybe "-")
    "FILE")
   "print the alcor command list for reaching the solution,\
