...
module Alcor.HTools.Backend.IAlloc
  ( readRequest
  , runIAllocator
  , processRelocate
  , loadData
...
import qualified Alcor.HTools.Cluster.AllocateSecondary as AllocSecondary
...
import qualified Alcor.HTools.Nic as Nic
import qualified Alcor.Constants as C
import Alcor.HTools.AlgorithmParams (AlgorithmOptions(algRestrictToNodes))
import Alcor.HTools.CLI
import Alcor.HTools.Loader
import Alcor.HTools.Types
import Alcor.JSON (maybeFromObj, JSRecord, tryFromObj, toArray, asObjectList,
                    readEitherString, fromJResult, fromObj, fromObjWithDefault,
                    asJSObject)
import Alcor.Types ( EvacMode(ChangePrimary, ChangeSecondary)
...
parseData :: ClockTime -- ^ The current time
          -> String -- ^ The JSON message as received from Alcor
...
parseData now body static_n_mem = do
  ...
  -- more than one hypervisor so in practice this should be fine. Alcor 2.17
